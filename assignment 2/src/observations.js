...
const { results, total_results } = require('./data');
...
function observationSummary(data) {
  for (let index = 0; index < data.results.length; index++) {
    console.log(
      `#${results[index].id} (${results[index].observed_on_details.date}) ${results[index].species_guess}`
    );
  }
}
...
function observationSummary2(data) {
  data.results.forEach((results) => {
    console.log(`#${results.id} (${results.observed_on_details.date}) ${results.species_guess}`);
  });
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  if (
    ...
    for (const a of data.results) {
      if (a.geoprivacy !== null) {
        upper = a.geoprivacy.toUpperCase();
      }
      if (a.geoprivacy !== null) {
        if (a.geoprivacy === geoPrivacy || upper === geoPrivacy) {
          obj = a;
          arr[c] = obj;
          c++;
        }
      } else if (a.geoprivacy === null && null === geoPrivacy) {
        obj = a;
        arr[c] = obj;
        c++;
      }
...
function transformObservation(original) {
  ...
  let a = original.location;
  a = a.split(',');
  let b = new Array();
  b.push(parseFloat(a[1]));
  b.push(parseFloat(a[0]));
  let d = new Array();
  let c = 0;
  for (let i = 0; i < original.photos.length; i++) {
    d.push(original.photos[i].url);
    c++;
  }
  let obj = {
    id: original.id,
    speciesGuess: original.species_guess,
    isResearchQuality: tf,
    coords: b,
    photos: d,
    photosCount: c,
...
function transformObservations(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    arr[i] = transformObservation(data.results[i]);
  }
  return arr;
}
...
function transformObservations2(data) {
  let arr = new Array();
  arr = data.results.map(function (element) {
    return transformObservation(element);
  });
  return arr;
}
...
function getObservationsById(data, ...ids) {
  ...
  ids.forEach(function (element) {
    arr[count] = data.results.find(function (a) {
      if (a.id === element) {
        return a;
      }
    });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (keys.includes('eq')) {
    arr = data.results.filter(function (element) {
      if (element.positional_accuracy === options.eq) {
        return element.positional_accuracy;
      }
    });
    ...
    if (keys[0] === 'gt' && keys[1] === 'lt') {
      arr = data.results.filter(function (element) {
        if (
          element.positional_accuracy >= options.gt &&
          element.positional_accuracy <= options.lt
        ) {
          return element.positional_accuracy;
        }
      });
    ...
    if (keys[0] === 'gt') {
      arr = data.results.filter(function (element) {
        if (element.positional_accuracy >= options.gt) {
          return element.positional_accuracy;
        }
      });
    } else {
      arr = data.results.filter(function (element) {
        if (element.positional_accuracy < options.lt) {
          return element.positional_accuracy;
        }
      });
    }
...
function getTaxonPhotos(data) {
  ...
  for (let o = 0; o < data.results.length; o++) {
    if (data.results[o].taxon != null && data.results[o].taxon.default_photo != null) {
      let s = data.results[o].taxon.default_photo.id;
      obj = {
        original:
        ...
        medium:
          ...
          '1384377507',
        large:
          'https://static.inaturalist.org/photos/' + s + '/' + 'large' + '.' + 'jpg?' + '1384377507'
      };
      arr.push(obj);
    }
  }
...
function getUserStats(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    counter++;
    obs = obs + data.results[i].user.observations_count;
    journal = journal + data.results[i].user.journal_posts_count;
    species = species + data.results[i].user.species_count;
  }
  let obj = {
    count: counter,
    totals: { observations: obs, journals: journal, species: species },
    averages: {
      observations: obs / counter,
      journals: journal / counter,
...
function extractTimeZones(data) {
  let arr = [];
  for (let i = 0; i < data.results.length; i++) {
    arr.push(data.results[i].observed_time_zone);
  }
  let unique = [];
  arr.forEach((element) => {
...
function extractTimeZones2(data) {
  ...
  let o = 0;
  for (let i = 0; i < data.results.length; i++) {
    obj.add(data.results[i].observed_time_zone);
  }
  let arr = new Array();
